...
int main(int argc, char *argv[])
{
	...
	//printf("argv[0]:%s,exe_duration:%d,\tnr_parallel_threads:%d,lock_delay:%d,rate:%.0f,\t\t", argv[0], exe_duration,nr_parallel_threads, lock_delay,1.0 * lock_counter / exe_duration);
	/*int j = 0;
	for (j = 0; j<nr_parallel_threads; j++) {
		printf("\nThe thread %d,count:%d\n", j, thread_param[j].count);
	}*/
	printf("%ld,",lock_counter);
